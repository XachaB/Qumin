...
def _draw_heatmap(*args, cmap=None, feat_order=None, dense=False, **kwargs):
...
def entropy_heatmap(results, md, cmap_name=False,
    ...
    df.loc[:, 'reverse'] = df.measure.str.contains('accuracy') & (df.type == "value")
    df.loc[:, 'type'] = df.type.replace({'value': 'Result', 'n_pairs': 'Number of pairs'})

    def rename(x):
        names = {'cond_entropy': 'H(pattern|classes)',
                 'accuracy': 'P(success|classes)'}
        suffix = " (debug)" if '_debug' in x else ""
        new_name = names[x] if x in names.keys() else x
        return new_name + suffix

    df.loc[:, 'measure'] = df.measure.apply(rename)
    ...
    cg.map_dataframe(_draw_heatmap, 'predictor', 'predicted', 'value', 'reverse',
                     ...
                     dense=dense, square=True, cbar=True,

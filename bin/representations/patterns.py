...
class Pattern(object):
    ...
    def _from_str(cls, cells, string):
        ...
        try:
            ...
            message = "I can't create a pattern from this: {}. Maybe the pattern has been exported with str and not repr ?".format(string)
...
class BinaryPattern(Pattern):
    ...
    def _create_regex(self):
        ...
        def make_transform_repl(a, b): return lambda x: Segment.get_from_transform(x,(a,b))
        def make_sub_repl(chars): return lambda x:chars
        ...
        for left,right in zip(self.alternation[c1], self.alternation[c2]):
            alternances.append(list(zip_longest(iter_alternation(left),iter_alternation(right),fillvalue=(False,""))))
        ...
        if nb_subs == 0 and (not self.applicable(form, from_cell)):
            if raiseOnFail:
                raise NotApplicable("The context {} from the pattern {} and cells {} -> {}"
                                ...
                                "".format(restore_string(self._regex[from_cell].pattern),self,from_cell,to_cell, form))
            else:
                return None
        return string
        ...
        for a,b in pairs:
            if a and b :
                if (l,a,b) in p.lexemes:
                    ...
                    correct_one = (p.apply(a, cells,raiseOnFail=False) == b) and (p.apply(b, cells[::-1],raiseOnFail=False) == a)
                    if correct_one:
                        correct += 1
                        p.lexemes.add((l,a,b))
        return correct
...
def _pat_auto_align(row, cells, collection, align_func):
...
def _pat_align_simple(row,cells,collection,align_func):
    a, b = row
    if row[0] == row[1]: # If the lists are identical, do not compute product
        pairs = [(x,x) for x in row[0]]
    else:
...
def find_suffixal_patterns(paradigms, **kwargs):
    ...
    Arguments:
        ...
        align_func (func, optional):
            Function used to align forms. From: align_baseline, align_left, align_right.
        categorical (bool, optional):
            has a inpact on output type:
            if categorical: cells are filled with str
            else: cells are filled with patterns
    ...
    Returns:
        ...
        (tuple):
            **patterns,pattern_dict**. Patterns is the created
            ...
            pat_dict is a dict mapping a column name to a list of patterns.
    """
    return _find_patterns_aligned(paradigms, categorical=False, align_func=alignment.align_baseline)

def _find_patterns_aligned(paradigms, categorical=False, align_func= alignment.align_left):
    r"""Find Patterns in a DataFrame according to some alignment function....
    ...
    def find_patterns_col(column, pat_dict, progress):
        pattern_collection = defaultdict(list)
        progress.increment()
        a,b = column.name

        paradigms_sets[[a, b]].apply(_pat_align_simple,
                                         axis=1,
                                         args=((a, b),
                                               pattern_collection,
                                               align_func))
    ...
    patterns = patterns.apply(find_patterns_col, axis=0, args=(pat_dict,progress))

    if categorical:
        patterns = patterns.applymap(str)
...
def find_levenshtein_patterns(paradigms, **kwargs):
    ...
    In the following example, the inventory is equivalent to the DataFrame of patterns,
    as we consider only one lexeme. For the same reason, the example cannot illustrate merging of patterns.

    Example:
        >>> df = pd.DataFrame([["amEn", "amənõ", "amənE"]],
             columns=["prs.1.sg", "prs.1.pl", "prs.2.pl"],
             index=["amener"])
        >>> df
               prs.1.sg prs.1.pl prs.2.pl
        amener     amEn    amənõ    amənE
        >>> pats, inventory = find_patterns(df)
        >>> pats
               (prs.1.sg, prs.1.pl) (prs.1.sg, prs.2.pl) (prs.1.pl, prs.2.pl)
        amener    E_ ⇌ ə_ɔ̃ / am_n_     E_ ⇌ ə_E / am_n_       ɔ̃ ⇌ E / amən_
        >>> inventory
        {('prs.1.sg', 'prs.2.pl'): [E_ ⇌ ə_E / am_n_ <1.0>],
...
def _find_auto_patterns(paradigms, categorical=False, align_func=alignment.align_levenshtein,optim_mem=False):
    ...
    def find_patterns_col(column, progress, pat_dict):
        ...
        col.apply(_pat_auto_align, axis=1, args=((c1, c2), collection, align_func))
    ...
    patterns_df = patterns_df.apply(find_patterns_col, axis=0, args=(progress, pat_dict))

    if categorical:
        patterns_df = patterns_df.applymap(str)

    return patterns_df, pat_dict

def find_applicable(paradigms, pat_dict, categorical=False):
    ...
    Arguments:
        ...
        categorical  (bool, optional): has a inpact on output type:
            if categorical: cells are filled with strings
            representing the class
            else: cells are filled with tuples of patterns.
    ...
    def _applicable_patterns_tuple(*args):
        return tuple(_iter_applicable_patterns(*args))

    def _applicable_patterns_str(*args):
        return ", ".join(str(pat) for pat in _iter_applicable_patterns(*args))
    ...
    if categorical:
        applicable = _applicable_patterns_str
    else:
...
def find_local_alternations(paradigms, **kwargs):
    ...
    Example:
        ...
        >>> find_local_alternations(df)
               (prs.1.sg, prs.2.sg) (prs.1.sg, prs.3.sg) (prs.1.sg, prs.1.pl)
        amener              __ ⇌ __              __ ⇌ __          _E_ ⇌ _ə_ɔ̃
               (prs.1.sg, prs.2.pl) (prs.1.sg, prs.3.pl) (prs.2.sg, prs.3.sg)
        amener           _E_ ⇌ _ə_E              __ ⇌ __              __ ⇌ __
               (prs.2.sg, prs.1.pl) (prs.2.sg, prs.2.pl) (prs.2.sg, prs.3.pl)
        amener          _E_ ⇌ _ə_ɔ̃           _E_ ⇌ _ə_E              __ ⇌ __
               (prs.3.sg, prs.1.pl) (prs.3.sg, prs.2.pl) (prs.3.sg, prs.3.pl)
        amener          _E_ ⇌ _ə_ɔ̃           _E_ ⇌ _ə_E              __ ⇌ __
               (prs.1.pl, prs.2.pl) (prs.1.pl, prs.3.pl) (prs.2.pl, prs.3.pl)
        amener             _ɔ̃ ⇌ _E          _ə_ɔ̃ ⇌ _E_           _ə_E ⇌ _E_
...
def make_pairs(paradigms):
    ...
    # Pairing cells
    pairs = combinations(paradigms.columns, 2)
...
def max_identity_pattern(cells):
    p = Pattern(cells, [""]*len(cells))

...
def k_fold_mean_accuracy(paradigms, l, step, k, train_funcs, foldback, bipartite=False, split=False, log=None,
                         **kwargs):
...
def bipartite_accuracy_twocols(paradigms, *args, **kwargs):
    ...
    return {"accuracy": sum(prediction_rightA & prediction_rightB) + sum(prediction_leftA & prediction_leftB),
            "count": countA + countB}

#TODO: SOmething is wrong here, paradigms is not used !
...
def main(args):
    ...
    try:
        ...
        paradigms = create_paradigms(args.paradigms, segcheck=True, fillna=False, merge_cols=True, overabundant=False,
                                     defective=True)
        ...
        if args.bipartite is not None:
            paradigms2 = create_paradigms(args.bipartite, segcheck=True, fillna=False, merge_cols=True,
                                          overabundant=False, defective=True)
            index = list(set(paradigms.index) & set(paradigms2.index))
            assert len(
                index) > 0, "For bipartite systems, you need both file to have at least some common indexes. Aborting."
        ...
        results = k_fold_mean_accuracy(paradigms, l, step, args.kfolds, train_funcs, args.foldback,
                                       bipartite=args.bipartite, split=args.split, log=logfile, features=features)
